...
for j=1:size(c,1)
    ...
    for k=1:size(sess,1)
    	...
    	[a_mat,flex]=network_diags(conn_cell_cat,8,500,1,.7);
    	save(char(strcat(c(k),sess(k).name,'/a_mat_learning')),'a_mat');
    	save(char(strcat(c(k),sess(k).name,'/flex_learning')),'flex');
