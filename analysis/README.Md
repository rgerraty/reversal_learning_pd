...
for j=1:size(c,1)
    ...
    for k=1:size(sess,1)
        ...
        if  ~isempty(conn_cell_cat)
    		[q(:,:,j,k),q_tmp(:,:,j,k),mean_flex(:,:,j,k),mean_flex_tmp(:,:,j,k),...,std_flex(:,:,j,k),std_flex_tmp(:,:,j,k)]=...
    		Q_grid(conn_cell_cat,10,res_pars,c_pars);
...
%save group averages
savedir='/data/engine/abuch/NETPD/gridopt/';
