...
for ii = 1:length(conn_cells),conn_cells{ii}(isnan(conn_cells{ii})) = 0;end
...
for i=1:sim
	...
	k=1
	for b=1:blocks
		flex_tmp(:,b,i)=flexibility(S_tmp(:,k:round(b*size(S_tmp,2)/blocks),i)');
		k=round((b*size(S_tmp,2)/blocks))+1
	end
end;
