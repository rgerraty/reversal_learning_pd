...
function aq=ReversalTask_Aquisition(rewCat, day, med, scanned, folder_name, SubjectNumber,list, prob,blockLength, nTrials,trigger,buttonBox,kb)
...
try
    ...
    onsetlist=load(sprintf('pilotonsets_%d.mat',list));
...
%                 Screen('FrameRect',window, [0 255 0], StimBoxFrame, 6); %make frame green
    ...
    %% Save frequently and set blocks
        ...
        %and numbers)
        fid = fopen(sprintf('%s/%d.csv',folder_name,SubjectNumber),'w')
